...
| full `pytest -q` | **170 passed** (163 + 7) on 9 consecutive runs. One earlier run — the first after the negative-control file swaps — reported 1 error; its text was not captured (my command kept only the summary line) and it did not reproduce in the 9 runs, 3 of them with backend hot-reloads forced mid-run. Recorded as unexplained, not as a pass. |
