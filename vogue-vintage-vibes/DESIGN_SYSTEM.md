...
| `@tanstack/react-table` v8 | **installed** (8.21, decision D8) — the one admin data table, `admin/AdminDataTable.tsx` (F4.3) | spec |
...
| `admin/ProductImageManager.tsx` | Gallery (AB-FE-04): drop/pick files → one-at-a-time upload with a local preview and `role="progressbar"` (`api.uploadImage(file, onProgress)`, B6.18: a presigned MinIO **POST policy** — multipart POST, server-side 5 MB / `image/*` limits, MinIO's `EntityTooLarge` / `AccessDenied` shown as Persian in the error tile), error tile with «تلاش دوباره» / «کنار گذاشتن», drag tiles or arrows to reorder, ★ makes an image primary (`value[0]`, «اصلی» badge), delete, URL entry; signed URLs cached per ref (no re-sign on reorder); `onBusyChange` lets the form block «ذخیره محصول» while uploading. |
| `admin/AdminDataTable.tsx` | F4.3 (D8): the one admin data table — `@tanstack/react-table` with controlled search / filter chips / sort / server page, row selection → floating bulk-action bar, `CopyValue` (one-click copy for order numbers, phones, tracking codes, emails), loading / empty / error + retry. Screens map its state to the API query; never add a second table abstraction. |
...
|---|---|---|
| [FE-01] `admin/AdminLayout.tsx` | `w-64` right sidebar, 18px lucide icons, terracotta active edge, topbar with `Cmd+K` search + avatar/role/logout | **exists** (`admin.tsx`, F4.2): sticky topbar (h-16, blur) with quick search → `/shop?q=`, role badge («مدیر ارشد» / «مدیر» / «مدیر سفارش‌ها» / «پشتیبانی») + logout; right sidebar in a rounded card (desktop) / right-side Sheet (mobile); 18px lucide icons, active `bg-terracotta/10`, per-tab badges fed by `/admin/kpis`; navigation is **role-gated** per `ROLE_TAB_KEYS` (mirrors backend `ROLE_CAPABILITIES`, B5.4); a non-staff account gets no tabs at all, only the no-access notice (F5.10). **AB-FE-01:** breadcrumbs «پنل مدیریت › section» (one `aria-current` crumb; root crumb hidden on phones), initials avatar with a profile `DropdownMenu dir="rtl"` (name, email, role; «حساب کاربری من», «اعلان‌ها»), storefront preview in a new tab (icon-only below `md`), and a working Ctrl/⌘+K (`event.code === "KeyK"`, so it works on the Persian layout) that focuses the visible quick-search box |
| [FE-02] `admin/AdminDataTable.tsx` | `@tanstack/react-table` v8, server-side pagination/sort/filter, bulk-action bar, CSV/Excel export | **exists** (F4.3): controlled search (debounced) / filter chips / sort (`aria-sort`, dual arrows) / server page, `bg-muted/30` header, row selection → floating dark bulk bar (reset on any page/filter/sort change), `CopyValue` copy helper, skeleton / empty / error + retry states, scrolls inside its own box on phones; used by `/admin/orders` (bulk status) and `/admin/users`. Export stays in its own panel (AB-FE-02) |
...
| Profile form sections (`account.index.tsx`, F5.20) | The profile tab is one page of titled sections («اطلاعات شخصی»، «اطلاعات تماس»، «اطلاعات هویتی»، «سایز من») — section headings, no invented card language. Inputs are the shared `form.tsx` fields with Persian labels and unit hints («قد (سانتی‌متر)»); gender is a `ui/select` with «نامشخص» default; email is read-only text with a «تأییدنشده» chip (`bg-sand … text-terracotta` metadata-chip style). Each section saves independently: disabled submit while saving, success/error toast, **no optimistic writes — the server response is the canonical state**. Omitted vs cleared is handled by sending `null` for emptied optionals; the backend treats an omitted key as unchanged. |
...
| 7 | Admin sidebar shell `components/admin/AdminLayout.tsx`, `AdminDataTable.tsx`, `OrderDetailSheet.tsx`, `RefundActionDialog.tsx`, `admin.refunds.tsx`, `admin.coupons.tsx` | tab-shell `admin.tsx` (no `AdminLayout.tsx`); `admin/AdminDataTable.tsx` (F4.3), `admin/OrderDetailDrawer.tsx` (the spec's sheet, F4.4), `admin.refunds.tsx` and `admin.coupons.tsx` exist; no `RefundActionDialog.tsx` | **Mostly built** — F4.1–F4.5 in `plan/frontend-tasks.md` |
...
5. **The spec's admin surface is mostly built** — sidebar shell (F4.2), order
   ...
   missing: CRM 360 (F4.8/F4.6); the data grid is `AdminDataTable` (F4.3, used
   by orders and users) and the coupons manager exists (F4.5). `recharts` is now used (F2.6); RHF/zod are used by the
